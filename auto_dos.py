...
#	print '##### ENERGY SPLIT #####', split
	...
	for idx, e1 in enumerate( energy ):
		for jdx, e2 in enumerate( energy ):
			...
			xes_brd[idx] += xes[jdx] / math.pi * ( math.atan(( e2 - e1 + delta_e ) / gamma ) - math.atan(( e2 - e1 - delta_e ) / gamma ))
	...
	if res > 0.0 :
		...
		for idx, e1 in enumerate( energy ):
			for jdx, e2 in enumerate( energy ):
				...
				xes_final[idx] += xes_brd[jdx] * 2.0 * delta_e / math.sqrt( 2.0 * math.pi ) / sigma * math.exp( -( e1 - e2 )**2 / ( 2.0 * sigma**2 ))			
	else :
...
def sum_xes( file_list, sum_file = 'summed_xes', mult_list = [] ) :
	...
	for xes_file in file_list:
		...
		for line in xes_data :
			...
			if line.strip( '-,. 0123456789Nae\n' ) == '' :
				for jdx in range( len( line.split( ',' )) / 3 ):
	...
	for idx, xes_file in enumerate( file_list ):
		...
		for line in xes_data :
			...
			e.append( [float( line_data[3 * i] ) for i in range( len( line_data ) / 3 )] )
			x.append( [float( line_data[3 * i + 2] ) for i in range( len( line_data ) / 3 )] )
...
for calc_dir in calc_dir_list :
	...
	return_val = get_params( case_name + '.qtl'+updn, 'JATOM', [3,-1], 0, [' ','=','\n'] )
	...
	if ( atom_Z == [] ) or ( len( atom_Z ) != len( multiplicity )) or ( len( atom_Z ) != len( symmetry )) or ( len( atom_Z ) != len( atom_name )):
		print "PROBLEM!!!"
